...
`MiniTsAST.Printer`, which is where the printer of an expression lives.
...
namespace Language.TypeScript.MiniTsAST.Printer

open Language.JavaScript
open Language.JavaScript.Doc
open Language.JavaScript.MiniAST (encodeJSXText encodeJSXAttrString)
...
it is read at and the documents of its attributes.  `selfClosing` says
that the element has no children, `oneStringAttr` that its only attribute
is one whose value is a string without a line break, which prettier never
breaks the tag for, and `breaks` that one of the attribute values holds a
line break, which always breaks it.  The type arguments of a component,
`<Comp<string> />`, stand between the name and the attributes, and break
on their own once they no longer fit.
...
broken. -/
def jsxOpeningDocOf (name : String) (typeArgs : Doc) (selfClosing oneStringAttr breaks : Bool)
    (attrs : List Doc) : Doc :=
  let nameDoc := tx ("<" ++ name) ++ typeArgs
  if selfClosing && attrs.isEmpty then nameDoc ++ tx " />"
  else if oneStringAttr then
    .group (nameDoc ++ tx " " ++ Doc.concat attrs ++ tx (if selfClosing then " />" else ">"))
  else
...
end Language.TypeScript.MiniTsAST.Printer
